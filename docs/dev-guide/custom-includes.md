...
The head and body includes render on every cart and checkout page, so branch on `step` to target a single one. The summary include renders only in the single-page checkout summary, where `step` is always `single-page`.
...
**The summary include renders once, server side.** Quantities are read-only at the checkout, so a total it reads from `cart` cannot change while the customer is on the page. Anything that has to follow a shipping method or coupon change belongs in the Alpine state instead.
