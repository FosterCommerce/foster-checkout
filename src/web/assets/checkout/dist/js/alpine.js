...
const AddressVerification = (props) => ({
  ...
  signature(address) {
    return FIELDS.map(
      (attribute) => normalize(address[attribute], attribute)
    ).join("|");
  },
  ...
  suggestionFrom(data2, entered) {
    const validated = data2?.response?.validatedAddresses?.[0];
    if (data2?.success !== true || !validated) {
      return null;
    }
    const suggested = {
